...
class ViewController: UIViewController {
  ...
  func playSound(sound: String, node: SCNNode) {
    ...
    boxNode.geometry?.firstMaterial?.diffuse.contents = UIColor(_colorLiteralRed: 0.01 * Float(height), green: 0, blue: 1, alpha: 1)
  ...
  func gameOver() {
    ...
    let fullAction = SCNAction.customAction(duration: 0.3) { _,_ in
      ...
      if self.height <= 15 {
        mainCamera.camera?.orthographicScale = 2
      } else {
        mainCamera.camera?.orthographicScale = Double(Float(self.height) / mainCamera.position.y)
      }
    }
  ...
  func addNewBlock(_ currentBoxNode: SCNNode) {
    ...
    newBoxNode.geometry?.firstMaterial?.diffuse.contents = UIColor(_colorLiteralRed: 0.01 * Float(height), green: 0, blue: 1, alpha: 1)
  ...
  func addBrokenBlock(_ currentBoxNode: SCNNode) {
    ...
    if height % 2 == 0 && absoluteOffset.z > 0 {
      ...
      // 2
      if offset.z > 0 {
        brokenBoxNode.position.z = currentBoxNode.position.z - currentSize.z / 2
      } else {
        brokenBoxNode.position.z = currentBoxNode.position.z + currentSize.z / 2
      }
      ...
      // 3
      brokenBoxNode.physicsBody = SCNPhysicsBody(type: .static, shape: SCNPhysicsShape(geometry: brokenBoxNode.geometry!, options: nil))
      brokenBoxNode.geometry?.firstMaterial?.diffuse.contents = UIColor(_colorLiteralRed: 0.01 * Float(height), green: 0, blue: 1, alpha: 1)
